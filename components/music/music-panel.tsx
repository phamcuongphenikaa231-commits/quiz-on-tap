...
export function MusicPanel({ isOpen, onClose }: MusicPanelProps) {
  ...
  // Hydrate mounted state after mount (for createPortal)
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  // Handle iOS Safari visualViewport changes (for virtual keyboard)
  useEffect(() => {
  ...
  const panelContent = (
    <>
      ...
      >
        {/* Sticky Header (Sticky on mobile, clean on desktop) */}
        <div className="music-panel__sticky-header">
          ...
          <div className="music-panel__header">
            ...
            <button
              onClick={onClose}
              aria-label="Thu nhỏ trình phát nhạc"
        ...
        {/* Active Track Status & Mini Player Control */}
        {currentSource && (
            ...
            <div className="music-panel__player-header">
              <span className="music-panel__player-label">
                {currentSource.provider === "youtube" ? (
                  <><Youtube className="h-3.5 w-3.5 text-red-500" aria-hidden="true" /> YouTube</>
                ) : (
                  <><Music2 className="h-3.5 w-3.5 text-orange-500" aria-hidden="true" /> SoundCloud</>
                )}
              </span>
