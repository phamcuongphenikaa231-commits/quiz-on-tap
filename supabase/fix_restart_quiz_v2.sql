-- ============================================================
-- FIX RESTART QUIZ v2
-- Sửa lỗi runtime PostgreSQL: v_existing_attempt record chưa gán
-- Khi p_force_new = true, code truy cập v_existing_attempt.id
-- nhưng record chưa được SELECT INTO -> runtime error
-- ============================================================

begin;

-- Drop function overload cũ (2 tham số) để tránh xung đột
drop function if exists public.start_quiz_attempt_fast(uuid, text);

-- Tạo lại function với scalar variables thay vì record
...
declare
  ...
  -- Dùng scalar thay vì record để tránh lỗi field access khi chưa gán
...
begin
  ...
  -- Nếu p_force_new = true: bỏ qua, hai biến scalar giữ null -> tạo attempt mới
  ...
  -- Kiểm tra scalar null thay vì record.id để tránh runtime error
  if v_existing_attempt_id is not null then
    ...
    -- Lưu attempt_questions với thứ tự option mới (đảo ngẫu nhiên nếu shuffle_options)
